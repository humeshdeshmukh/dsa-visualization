...
export default function LinearProbingPage() {
  ...
  const insert = async (key: string, value: string) => {
    ...
    while (hashTable[index] !== null && !hashTable[index]?.isDeleted && !visited.has(index)) {
      ...
      setMessage(`Collision occurred. Probing next position: ${index}`);
      setHighlightIndices([index]);
      await sleep(500);
    }
  ...
  const search = async (key: string) => {
    ...
    while (hashTable[index] !== null && !visited.has(index)) {
      ...
      probes++;
      index = (index + 1) % tableSize;
      setMessage(`Key not found at ${index - 1}, probing next position: ${index}`);
      setHighlightIndices([index]);
      await sleep(500);
    }
  ...
  return (
    <VisualizerLayout
      title="Linear Probing Hash Table Visualization"
      description="Visualize how Linear Probing handles collisions by searching for the next empty slot in the array."
    >
      <VisualizationProvider>
        <div className="space-y-6">
          ...
          <div className="glass-dark border border-white/10 rounded-lg p-6 space-y-4">
            <h3 className="text-lg font-semibold text-white">Linear Probing Properties</h3>
            <div className="space-y-2 text-gray-300">
              <p><strong>Time Complexity:</strong></p>
              <ul className="list-disc list-inside mt-2 ml-4">
                <li>Average Case: O(1/(1-α)) where α is the load factor</li>
                <li>Worst Case: O(n) when many elements cluster</li>
              </ul>
              ...
              <ul className="list-disc list-inside mt-2 ml-4">
                ...
                <li>Good for high load factors</li>
              </ul>
              <p><strong>Disadvantages:</strong></p>
              <ul className="list-disc list-inside mt-2 ml-4">
                <li>Primary clustering</li>
                <li>Performance degrades as table fills</li>
                <li>Deletion requires tombstone markers</li>
              </ul>
            </div>
          </div>
        </div>
